import numpy as np
import cv2
from pymodules.seat_utils import rectangle_area, SeatStatus, get_overlap_rectangle
from pymodules.background_subtractor import BackgroundSubtractorMOG2
import time


class Seat:
    def __init__(self, initial_background, bb_coordinates, table_bb, ID, bg_sub_threshold=50, bg_alpha=0.99):
        ...
        if self.table_bb is None:
            # for i in self.bb_coordinates:
            print(self.bb_coordinates)
    ...
    def no_person_detected(self, seat_img, start):
    ...
    def become_empty(self):
        ...
        print('{}: AVAILABLE'.format(self.id))


    def become_on_hold(self, start):
        ...
        print('{}: RESERVED'.format(self.id))
        # print('Time: {:.4f} seconds'.format(time.time() - start))


    def update_chair_bb(self, bbox): # Dont Understand
